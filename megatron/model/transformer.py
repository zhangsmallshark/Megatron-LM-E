...
class CoreAttention(MegatronModule):
    ...
    def __init__(self, layer_number,
                 attn_mask_type=AttnMaskType.padding, intra_partitions=1):
        ...
        self.hidden_size_per_partition = core.utils.divide(
            self.hidden_size_per_partition, intra_partitions)
...
class ParallelAttention1(MegatronModule):
    ...
    def __init__(self, init_method,
        ...
        self.core_attention = CoreAttention(self.layer_number,
                                            self.attn_mask_type, intra_partitions=self.intra_partitions)
        self.checkpoint_core_attention = args.recompute_granularity == 'selective'
        ...
        # Output.
        self.dense0 = tensor_parallel.RowParallelLinear1(
            projection_size,
            args.hidden_size,
            # self.intra_partitions,
            2,
        ...
        self.dense1 = tensor_parallel.RowParallelLinear1(
            projection_size,
            args.hidden_size,
            # self.intra_partitions,
            2,
    ...
    def forward(self, hidden_states, attention_mask,
        ...
        # p_outputs = []
        # p_biases = []
        # current_stream = get_stream(0)
        # with torch.cuda.stream(current_stream):
        p_output0, p_bias0 = self.dense0(context_layer)

        # current_stream = get_stream(1)
        # with torch.cuda.stream(current_stream):
        p_output1, p_bias1 = self.dense1(context_layer)
            
        output = torch.cat((p_output0, p_output1), dim=-1)
        bias = torch.cat((p_bias0, p_bias1), dim=-1)
